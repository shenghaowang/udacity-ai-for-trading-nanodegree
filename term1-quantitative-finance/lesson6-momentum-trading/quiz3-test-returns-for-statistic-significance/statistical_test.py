...
def analyze_returns(net_returns):
    ...
    null_hypothesis = 0.0
    t_statistic, p_two_tailed = stats.ttest_1samp(net_returns, null_hypothesis)
    
    return t_statistic, p_two_tailed / 2
...
def test_run(filename='net_returns.csv'):
    ...
    # net_returns = pd.read_csv(filename, header=0, index_col=0).transpose().iloc[0]
