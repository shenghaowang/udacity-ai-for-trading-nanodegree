...
def print_dataframe(df, n_rows=10, n_columns=3):
    ...
    trace = go.Table(
        ...
        header = dict(
            values = [''] + list(formatted_df.columns.values),
            line = dict(color = helper.color_scheme['df_line']),
            fill = dict(color = helper.color_scheme['df_header']),
            font = dict(size = 13)
        ),
        ...
        cells = dict(
            values = formatted_df.reset_index().values.T,
            line = dict(color = helper.color_scheme['df_line']),
            fill = dict(color = [helper.color_scheme['df_header'], helper.color_scheme['df_value']]),
